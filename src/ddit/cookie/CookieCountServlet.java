...
public class CookieCountServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(cookies!=null) {
			for(Cookie cookie : cookies) {
				...
				if("count".equals(name)) {
					String value = cookie.getValue();	// 쿠키의 value값(현재의 count 값) 구하기
		...
		out.println("<html><head><meta charset='utf-8'><title>쿠키 카운트</title>");
		out.println("<body>");
		out.println("<h3>어서오세요. 당신은 " + count+"번째 방문입니다.</h3>");
		
		out.println("<a href='"+request.getContextPath()+"/cookieCountServlet.do'>카운트 증가하기</a>");
		out.println("<a href='"+request.getContextPath()+"/ddit/cookie/cookieTest02.jsp'>시작문서로 가기</a>");
